...
def test_render_element():
    ...
    # making sure there is only one start and one end html tag
...
def test_li():
    ...
    print(file_contents)
    assert file_contents == '<li style="color: red">\nThis is the second item\n</li>\n'
    # assert False


def test_h():
    h = H(1, "this is some text")
    file_contents = render_result(h).strip()
    print(file_contents)

    assert file_contents == '<h1>this is some text</h1>'
    assert ("this is some text") in file_contents
    assert file_contents.startswith("<h1>")
    assert file_contents.endswith("</h1>")

# TODO: Finish Steps 8 and 9
# #####################
# # indentation testing
